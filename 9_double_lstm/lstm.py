...
table = pd.read_csv('stl7resid_component_7.csv', parse_dates=True, index_col='timestamp', date_parser=dateparse)
table = (table.resample('D').mean().interpolate('linear'))
index=table.index

target = 'seasonal'

trainData = np.array(table[target])
...
writeResult(testY,testPredict,index,'stl1resid_'+target+'_lstm_pre')
