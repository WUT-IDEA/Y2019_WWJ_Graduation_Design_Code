...
if __name__ == '__main__':
    pre = readData('prophet_forecast.csv')
    pre_arr = np.array(pre['yhat'])

    actual = readData('purchase_table.csv')[-31:]
    actual_arr = np.array(actual['y_log'])
    print(actual)

    score = score_action(actual_arr,pre_arr)
    print("得分：",score)

    score2 = score_action2(actual_arr, pre_arr)
    print("得分2：", score2)
